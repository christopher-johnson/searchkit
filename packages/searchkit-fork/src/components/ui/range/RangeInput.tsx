...
export class NumberInput extends React.Component<any, any> {
  ...
  onChange(e) {
    ...
    this.setState({value})
    if (NumberInput.isValid(value) && onChange) {
  ...
  render() {
    ...
    return (
      <form className={bemBlocks.container().mix(className)} onSubmit={this.handleSubmit}>
        <NumberInput ref="min" className={bemBlocks.container("input")} value={minValue} field="min" onChange={this.handleInputChange}
          placeholder={translate('range.min') || minPlaceholder}/>
        <div className={bemBlocks.container("to-label")}>{translate('range.to') || '-'}</div>
        <NumberInput ref="max" className={bemBlocks.container("input")} value={maxValue} field="max" onChange={this.handleInputChange}
