...
export class RangeSlider extends React.PureComponent<any, {}> {
  ...
  render() {
    const {
      mod, className, step, marks, rangeFormatter,
    ...
    return (
      <div className={bemBlocks.container().mix(className)}>
        <Range min={min} max={max} marks={marks || {
          [min]: rangeFormatter(min),
          [max]: rangeFormatter(max)
        }} tipFormatter={rangeFormatter} range={true} step={step} value={[minValue, maxValue]} onChange={this.onChange}
