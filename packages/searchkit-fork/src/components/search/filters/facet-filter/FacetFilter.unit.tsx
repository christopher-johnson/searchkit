...
import {FacetAccessor, SearchkitManager, Utils} from "../../../../core"
import {ItemComponent, Toggle} from "../../../ui"
...
describe("Facet Filter tests", () => {
  ...
  beforeEach(() => {
    ...
    this.createWrapper(
      <FacetFilter field="test" id="testId" title="test title" size={3} countFormatter={(count) => "#" + count} include={"title"}
        exclude={["bad", "n/a"]} operator="OR" orderKey="_count" orderDirection="desc" translations={{"facets.view_all": "View all facets"}}
  ...
  it("show more options", () => {
    ...
    this.accessor.getMoreSizeOption = () => {
      return option
    }
  ...
  it("should work with a custom itemComponent", () => {
    this.createWrapper(
      <FacetFilter itemComponent={({label, count}) => <div className="option">{label} ({count})</div>} field="test" id="testId"
        title="test title" searchkit={this.searchkit}/>
    )
  ...
  it("should work with custom components", () => {
    ...
    this.createWrapper(
      <RefinementListFilter containerComponent={container} listComponent={Toggle}
        itemComponent={(props) => <ItemComponent {...props} showCount={true}/>} field="test" id="testId" title="test title"
        bucketsTransform={(buckets) => _.reverse(buckets)} searchkit={this.searchkit}/>
    )
    expect(this.wrapper).toMatchSnapshot()
  })
